...
- **Smart Wallet API v2:** The Smart Wallet API v2 (previously referenced as Smart Wallets, now deprecated) enables Developers to Authenticate and access a `jwt` string which they can then parse to enable **sessions** for users. This enables users who Authenticate in the DApp provided by the Developer and are recognised upon sign out and sign in without the need to re-authenticate. The endpoints available are:
  - `Create Wallet`
  - `Send Single & Batch Transactions`
  - `Get Wallet Action`

Below is an example using the JS SDK to create a Smart Contract Account
...
The FuseBox Flutter SDK is available as a Flutter [package](https://pub.dev/packages/fuse_wallet_sdk) for Mobile applications development. The Package contains all the endpoints available in the REST API.
...
// Create a project and get an API Key here: https://developers.fuse.io
final publicApiKey = 'YOUR_PUBLIC_API_KEY';
final fuseSDK = await FuseSDK.init(publicApiKey,credentials,
// Optional: Specify baseUrl. Default is api.fuse.io.
