...
[FuseBox](https://github.com/fuseio/fusebox-backend) is a Open Source Developer Tool designed for constructing applications on the Fuse Network. It provides a set of `methods` that are available to Developers via a **REST API**, a **Flutter SDK** and a **TypeScript SDK**. Thus making is easier for Developers to interact wiht the Fuse Blockchain without the need to make direct RPC calls, and eliminating the need to use a node service provider.

Developers can also build using FuseBox by cloing the **GitHub** repository as it is Open Source, making Fuse Blockchain the ONLY Blockchain enabling this for Developers.

[FuseBox Backend](https://github.com/fuseio/fusebox-backend) is developed with the NestJS framework.
...
- Smart Wallet API v2: This API enables the creation and administration of smart wallet accounts built on the ERC4337 standard.
