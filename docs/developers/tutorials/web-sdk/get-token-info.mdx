...
The application has two primary functions: [`GET Token`](/docs/developers/fusebox/sdk/10-get-token-details) and [`Token Price`](/docs/developers/fusebox/sdk/15-get-token-price) details. Both actions will be carried out using the FuseBox Web SDK. We will also use the EthersJS Library for particular Wallet methods. Install the EthersJS library; we will use v6.
...
This is a guide to `GET` requests using the FuseBox SDK. In this example, we are using the **[GET Token Details](/docs/developers/fusebox/sdk/10-get-token-details)** and **[GET Token Price](/docs/developers/fusebox/sdk/15-get-token-price)** methods to illustrate how a developer can call the methods and build a simple interactive user interface.
