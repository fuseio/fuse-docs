...
In this demo, we are not going to focus on the aesthetics of the application, rather, we are going to focus on the implementation the creation of a Smart Contract Wallet using FuseBox, funding the Wallet and carrying out a Transfer. Here is a Basic UI for the application, update the `main.dart` file:
