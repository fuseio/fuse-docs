---
title: Connect to Ember
sidebar_position: 0
---

import ConnectButton from "../../components/ConnectToEmberButton/ConnectButton";

## Connect to Ember

Add Ember to your wallet or development environment to get started.

Use the information below to connect and submit transactions to Ember.

## Testnet

| Property        | Description                                                                                   |
| --------------- | --------------------------------------------------------------------------------------------- |
| Name            | Flash Testnet                                                                                 |
| Description     | The public testnet for Ember.                                                                 |
| Chain ID        | 10920                                                                                         |
| RPC URL         | https://floral-green-gadget.fuse-flash.quiknode.pro/0b560e99d1a605b43771fc313dbc3e7054df160e/ |
| Explorer URL    | https://fuse-flash.explorer.quicknode.com/                                                    |
| Currency Symbol | ETH                                                                                           |

<ConnectButton />

Once connected, use a [bridge](https://fuse-flash-api.bridge.quicknode.com/) or [faucet](https://faucet.quicknode.com/fuse/flash) to get testnet funds into your wallet.

## Mainnet

Ember Mainnet is coming soon.
