...
Within the Console, you'll see a Revoke button alongside other information about each node you are managing. This enables users to revoke delegation of their nodes to an operator, so they can be delegated to another. Delegation is revoked immediately; there is no withdrawal period, so nodes can immediately be re-delegated.
