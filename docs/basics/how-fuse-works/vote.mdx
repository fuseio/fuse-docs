...
Fuse chain functionality can be changed by voting on the contracts implementations. New implementations can be deployed and opened to vote by validators for others to decide whether to accept/reject the changes.

## Open a new ballot

To open a new vote, a validator needs to call the "newBallot" function on the [voting contract](https://explorer.fuse.io/address/0x4c889f137232E827c00710752E86840805A70484) with the following parameters:
...
Everyone can check all the ballots created using the "getBallotInfo" function. This function receives two parameters:
...
Everyone can vote on open ballots after the start block has passed until the final block.
