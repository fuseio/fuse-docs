...
A new NextJS project is created with a pages directory containing an `index.tsx` file.
...
In this step, we will test calling the API and parsing the response to the web browser console. Add the following code to the `index.tsx` file inside the `Home() {}` Component.
...
Once the API data is fetched, it is displayed in a table format using Tailwind CSS for styling. This part of the UI will render conditionally based on whether the `result` state is populated.
