...
In this article, we will use the Fuse Wallet SDK **[Swap Tokens SDK](../sdk/swap-tokens)** method to create an App where users can Swap Tokens to the Fuse Native Token. In this example App, users can select USDC, USDT, and VOLT to swap to Fuse Tokens.
...
The app's basic functionality is to swap from one token to another, in this first example, from USDC to FUSE. The method is:
...
At this point, you can confirm that the SWAP functionality works! The next step is a bonus step to improve the User Experience, and we will update the code to include a Dropdown for selecting currencies and an input field to specify a particular amount. Add the Token address variables:
